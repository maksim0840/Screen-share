...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	// hPrevInstance - устарер (испольщовался для 16 бит системы)
	...
	wc.cbClsExtra = 0; // дополнительное выделение памяти в класе окна (с.да засунуть размер того что запихивать)
	wc.cbWndExtra = 0; // дополнительное выделение памяти в класе окна (с.да засунуть размер того что запихива
	wc.hbrBackground =  (HBRUSH)(COLOR_WINDOW + 1);
	wc.hCursor = LoadCursor(NULL, IDC_ARROW); // handle курсора
	wc.hInstance = hInstance; // мета данные об окне
	wc.hIcon = LoadIcon(hInstance, IDI_APPLICATION); // ИКОНКА ПРИЛОЖЕНИЯ СНИЗУ НА ДИСПЕТЧЕРЕ (не файла)
	...
	wc.style = CS_VREDRAW | CS_HREDRAW; // стили окна (комбинация флагов (больше флагов можно посмотреть в документации())
